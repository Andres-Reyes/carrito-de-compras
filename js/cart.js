...
function remove(id) {
    ...
    for (let index = 0; index < productsLocal.length; index++) {
        if (productsLocal[index].id == id) {
            ...
            total();
            for (let index2 = 0; index2 < con2.length; index++) {
                if (x == con2[index2]) {
...
function updateCart() {
    ...
    for (let index = 0; index < con2.length; index++) {
        ...
        <td><img style="width: 5rem;" src="${productsLocal[index3].img}
        "></td>
        ...
        <input style="width: 2rem;" id="${productsLocal[index3].id}"
        value="${productsLocal[index3].quantity}" disabled>
        <button class="waves-effect waves-light btn purple darken-4"
        onclick="addAmount(${productsLocal[index3].id})">+</button>
        </td>
        <td>$ ${productsLocal[index3].price*productsLocal[index3]
        .quantity}</td>
        <tr>
        `
    ...
    <td></td>
    <td>
    <button onclick="clean()" class=" yellow accent-4 waves-effect waves-light 
    btn">Clean</button>
    </td>
    <td>
    <button href="#modal1" class="modal-trigger green accent-4 waves-effect
    waves-light btn">Buy</button>
    </td>
    </tr>
...
(() => {
    ...
    <tr>
    <th >${index+1}</th>
    <td><button class="waves-effect waves-light btn red darken-4" onclick="remove
    (${productsLocal[index].id})">X</button></td>
    <td><img style="width: 5rem;" src="${productsLocal[index].img}"></td>
    <td>${productsLocal[index].name}</td>
    <td>
    <button class="waves-effect waves-light btn purple darken-4"
    onclick="reduceAmount(${productsLocal[index].id})">-</button>
    <input style="width: 2rem;" id="${productsLocal[index].id}" 
    value="${productsLocal[index].quantity}"disabled>
    <button class="waves-effect waves-light btn purple darken-4"
    onclick="addAmount(${productsLocal[index].id})">+</button>
    </td>
    ...
    <td>
    <button href="#modal1" class="modal-trigger green accent-4 waves-effect waves-light
    btn">Buy</button>
    </td>
    </tr>
        ...
        firebase.firestore().collection("sales").add({
            ...
            .then(() => {
                swal.fire({
                    ...
                    text: `su orden de compra es: ${order}`,
